...
data_folder = "C:/Users/fmava/Downloads/EEG Learning/MDD Dataset Practice"
csv_file = os.path.join(data_folder, "patients_with_meg.csv")
entry_to_start = 13
...
def get_connectivity_measures(methods: List[str],
    ...
    for method in methods:
        connectivity_file_path = os.path.join(data_folder, 'Files',
...
for patient_file in df['Filename'][entry_to_start:(entry_to_start + number_of_entries_to_access)]:
    ...
    file_name = os.path.join(data_folder, "Matlab Files", patient_file)
    ...
    # PREPROCESSING
    if os.path.exists(os.path.join(data_folder, 'Files', f'{file_number}_Depression_REST_CLEANED-epo.fif')):
        print(f'\nPatient {file_number} already exists, retrieving data now\n')
        # Can enable plotting if user wants to
        # epochs_ar.plot(scalings='auto')
    else:
        preprocess_patient_data(epochs, patient_file, file_number, data_folder)
